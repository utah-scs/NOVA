...
{
    /* Restore vm state from the packet */
    restore_vm_state(vm, mem);

    uint64_t pc = vm->state->pc;
    const struct ebpf_inst *insts = vm->insts;
    uint64_t regs[16];
    uint64_t *reg;
    uint64_t *stack = vm->state->stack;

    /* Restore registers from packet */
    memcpy(regs, vm->state->regs, sizeof(regs));
     ...
     * if (ret == RET_DMA_HELPER)
     *   return ret;
     *
     * Helper functions will return twice. Once with RET_DMA_HELPER
     ...
     * call with returning the status of the dma operation,
     * so that fallthrough branch is taken
     * and the program continues.
     *
     * r0 is set to DMA status, to achieve that.
     */
    reg[0] = get_dma_status(mem);
     
    if (pc == 0)
        reg[1] = (uintptr_t)mem;
    
    reg[10] = (uintptr_t)stack + sizeof(vm->state->stack);

    /* 
     * Update all regs with ptr to pkt with new pkt addr
     * r1 holds the base address of the old packet buffer
    ...
    if (pc != 0) {
        for (int i = 0; i < 16; i++) {
            ...
            if (reg[i] >= reg[1] && reg[i] <= reg[1] + mem_len) {
                if (reg[i] == reg[1]) {
                    /*printf("r%d: %lx | r1: %lx\n", i, reg[i], reg[1]);*/
                    reg[i] = (uintptr_t)mem;
                }
                else {
                    reg[i] = (uintptr_t)mem + (reg[i] - reg[1]);
                }
            }
        }
    }
...
#ifdef DEBUG
        debug_dump_inst(vm, cur_pc);
        dump_vm_state(vm, cur_pc);
#endif
    ...
    do { \
        ...
        case EBPF_OP_EXIT:
            *bpf_return_value = reg[0];
            return 0;
        case EBPF_OP_CALL:
            // If it's a preemped helper call then store vm state at packet tail
            if (preempted_helper(vm, inst.imm)) {
                store_vm_state(vm, regs, pc);
            }

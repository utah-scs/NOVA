...
// 32 bit mode or 64 bit mode for register operations
enum BIT_MODE {BIT_MODE_32, BIT_MODE_64 = 1};
...
/* b.eq instruction */
static void
emit_b_eq(struct jit_state *state, uint32_t offset) {
    emit_instruction(state, BR_Bcond | (offset << 5) | (0 << 4) | COND_EQ);
}

/* adr instruction */
static void
emit_adr(struct jit_state *state, enum Registers rn, uint64_t imm21) {
    assert((imm21 >> 21) == 0 || (imm21 >> 21) == INT64_C(-1));

    emit_instruction(state, 0x10000000 | ((imm21 & 0x3) << 29) | (((imm21 >> 2) & 0x3ffff) << 5) | rn);
}
 ...
 */
static void
emit_function_prologue(struct jit_state *state)
{
    /* All the callee save registers and FP, LR */
    uint32_t register_space =  _countof(callee_saved_registers) * 8 + 2 * 8;

    state->stack_size = (register_space + 15) & ~15U;
...
translate(struct ubpf_vm *vm, struct jit_state *state, char **errmsg)
{
    ...
    emit_function_prologue(state);

    /// Copy registers from the packet

    /*
     * r1 contains pointer to the xdp_md context
     * Load packet buffer address from the xdp_md context
     * r2 = xdp_md->data
     */
    emit_loadstore_immediate(state, LS_LDRX, map_register(2), map_register(1), 0);

    /* Load offset of VM_STATE in the packet buffer to temp_register */
    emit_movewide_immediate(state, BIT_MODE_32, temp_register, offsetof(req_pkt_t, state));
    
    /* 
     * Point temp_div_register to the start of the vm_state
     * temp_div_register = r2 + offset
     */
    emit_addsub_register(state, BIT_MODE_64, AS_ADD, temp_div_register, map_register(2), temp_register);

    /*
     * Restore PC in temp_register
     *
     * emit_loadstore_register as imm needs to be 1 byte
     * value for emit_loadstore_immediate
     */

    emit_movewide_immediate(state, BIT_MODE_32, offset_register, offsetof(struct vm_state, pc));
    emit_loadstore_register(state, LS_LDRX, temp_register, temp_div_register, offset_register);

    /*
     * Setup ubpf stack
     *
     * Stack space from packet buffer is used as ubpf stack
     * This eleminates the need to copy stack while saving/restoring context
     *
     * Point r10 to the end of the packet buffer stack
     * r10 = temp_div_register + offset of end of stack
     */
    emit_addsub_immediate(state, BIT_MODE_64, AS_ADD, map_register(10), temp_div_register, offsetof(struct vm_state, pc));
    
    /* If PC == 0, skip all these */
    emit_addsub_immediate(state, BIT_MODE_64, AS_SUBS, RZ, temp_register, 0);
    emit_b_eq(state, 27); // skip next 26 instructions
    
    /* 
     * After helper function call r1-r5 is set to unreadable.
     * Restore r6 - r9
     */
    emit_loadstore_immediate(state, LS_LDRX, map_register(6), temp_div_register, offsetof(struct vm_state, regs[6]));
    emit_loadstore_immediate(state, LS_LDRX, map_register(7), temp_div_register, offsetof(struct vm_state, regs[7]));
    emit_loadstore_immediate(state, LS_LDRX, map_register(8), temp_div_register, offsetof(struct vm_state, regs[8]));
    emit_loadstore_immediate(state, LS_LDRX, map_register(9), temp_div_register, offsetof(struct vm_state, regs[9]));
    
    /* Restore r1 from the packet buffer to offset_register which contains address of the old packet buffer */
    emit_loadstore_immediate(state, LS_LDRX, offset_register, temp_div_register, offsetof(struct vm_state, regs[1]));

    /* Restore bit vector from r3 to r0 which is used for register relocation */
    emit_loadstore_immediate(state, LS_LDRX, map_register(0), temp_div_register, offsetof(struct vm_state, regs[3]));
    
    /* 
     * Fix pointer to packet buffers and stack if the packet buffer address changes
     *
     * Only registers from r6-r9 needs to be fixed as they are valid after helper function call.
     *
     * 1. Check if the bit is set in the bit vector to identify if the register contains pointer
     * 2. Find offset from the base address: offset = address in register - old packet buffer address
     * 3. Add new packet buffer address with the offset: new address + offset
     */
    
    /*
     * Pseudo assembly
     *
     * test r0, N ; N = 1 or 2 or 4 or 8 etc.
     * je offset
     * sub rX, RCX ; X = 6,7,8,9
     * add rX, r2
    */

    /* relocate r6 */
    emit_logical_register(state, true, LOG_ANDS, RZ, map_register(0), 1);
    emit_b_eq(state, 3); // skip next 2 instructions
    emit_addsub_register(state, BIT_MODE_64, AS_SUB, map_register(6), map_register(6), offset_register);
    emit_addsub_register(state, BIT_MODE_64, AS_ADD, map_register(6), map_register(6), map_register(2));
    
    /* relocate r7 */
    emit_logical_register(state, true, LOG_ANDS, RZ, map_register(0), 2);
    emit_b_eq(state, 3); // skip next 2 instructions
    emit_addsub_register(state, BIT_MODE_64, AS_SUB, map_register(7), map_register(7), offset_register);
    emit_addsub_register(state, BIT_MODE_64, AS_ADD, map_register(7), map_register(7), map_register(2));
    
    /* relocate r8 */
    emit_logical_register(state, true, LOG_ANDS, RZ, map_register(0), 4);
    emit_b_eq(state, 3); // skip next 2 instructions
    emit_addsub_register(state, BIT_MODE_64, AS_SUB, map_register(8), map_register(8), offset_register);
    emit_addsub_register(state, BIT_MODE_64, AS_ADD, map_register(8), map_register(8), map_register(2));
    
    /* relocate r9 */
    emit_logical_register(state, true, LOG_ANDS, RZ, map_register(0), 8);
    emit_b_eq(state, 3); // skip next 2 instructions
    emit_addsub_register(state, BIT_MODE_64, AS_SUB, map_register(9), map_register(9), offset_register);
    emit_addsub_register(state, BIT_MODE_64, AS_ADD, map_register(9), map_register(9), map_register(2));

    
    /*
     * Preempted helper functions should call following way in the program:
     * 
     * ret = helper_func(args)
     * if (ret == RET_HELPER)
     *   return ret;
     *
     * Helper functions will return twice. Once with RET_HELPER
     * to tell caller that it's a preemped helper functions call
     * and take action accordingly.
     *
     * Next time the program will continue after the helper function
     * call with returning the status of the dma opearion,
     * so that fallthrough branch is taken
     * and the program continues.
     */

    /* 
     * Load offset of DMA_STATE in packet buffer to temp_div_register
     */
    emit_movewide_immediate(state, BIT_MODE_32, temp_div_register, offsetof(req_pkt_t, dma_req));

    /* 
     * Point temp_div_register to the start of the DMA_STATE in the packet buffer
     * temp_div_register = PKT_MEM + temp_div_register
     */
    emit_addsub_register(state, BIT_MODE_64, AS_ADD, temp_div_register, temp_div_register, map_register(2));

    /* 
     * Load status of the DMA operation in R0
     */
    emit_loadstore_immediate(state, LS_LDRB, map_register(0), temp_div_register, offsetof(dma_req_t, status));
    
    /*
     * jmp to restored PC
     */
    emit_unconditonalbranch_register(state, BR_BR, temp_register);
    ...
    for (i = 0; i < vm->num_insts; i++) {
        ...
        case EBPF_OP_CALL:
            /* 
             * If it's a preempted helper function call
             * generate code for saving states into packet
             *
             * Preempted helper functions should have following
             * signature:
             *
             * helper_func(uint64_t *pkt)
             */
            if (preempted_helper(vm, inst.imm)) {
                /* Load offset of VM_STATE in the packet buffer to temp_div_register */
                emit_movewide_immediate(state, BIT_MODE_32, temp_div_register, offsetof(req_pkt_t, state));
                
                /* 
                 * BPF_R1 holds the pointer to the packet buffer
                 * Point temp_div_register to the start of the VM_STATE in the packet buffer
                 * temp_div_register = PKT_MEM + temp_div_register
                 */
                emit_addsub_register(state, BIT_MODE_64, AS_ADD, temp_div_register, temp_div_register, map_register(1));
                
                /* Store registers BPF_R2 - BPF_R9 */
                emit_loadstore_immediate(state, LS_STRX, map_register(2), temp_div_register, offsetof(struct vm_state, regs[2]));
                emit_loadstore_immediate(state, LS_STRX, map_register(3), temp_div_register, offsetof(struct vm_state, regs[3]));
                emit_loadstore_immediate(state, LS_STRX, map_register(4), temp_div_register, offsetof(struct vm_state, regs[4]));
                emit_loadstore_immediate(state, LS_STRX, map_register(5), temp_div_register, offsetof(struct vm_state, regs[5]));
                emit_loadstore_immediate(state, LS_STRX, map_register(6), temp_div_register, offsetof(struct vm_state, regs[6]));
                emit_loadstore_immediate(state, LS_STRX, map_register(7), temp_div_register, offsetof(struct vm_state, regs[7]));
                emit_loadstore_immediate(state, LS_STRX, map_register(8), temp_div_register, offsetof(struct vm_state, regs[8]));
                emit_loadstore_immediate(state, LS_STRX, map_register(9), temp_div_register, offsetof(struct vm_state, regs[9]));
                
                /* 
                 * Load PC into temp_register
                 * PC in ARM64 is the address of current instruction
                 */
                emit_adr(state, temp_register, 0);

                /*
                 * PC in packet should point to the next instruction after call instruction
                 * PC = PC + offset from emit_adr to after the emit_call inst
                 */
                emit_addsub_immediate(state, BIT_MODE_64, AS_ADD, temp_register, temp_register, 36);
                
                /* Store PC into the packet */
                emit_movewide_immediate(state, BIT_MODE_64, offset_register, offsetof(struct vm_state, pc));
                emit_loadstore_register(state, LS_STRX, temp_register, temp_div_register, offset_register);
            }

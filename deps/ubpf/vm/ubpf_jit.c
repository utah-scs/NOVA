...
#ifdef DUMP_JIT
    // Dump to file
    printf("Number of bytes JIT: %lu\n", jitted_size);
    int fd = open("jit.bin", O_CREAT | O_RDWR, S_IRWXU);
    if (fd > 0) {
      int w_ret = write(fd, buffer, jitted_size);
      printf("Wrote %d bytes to jit.bin\n", w_ret);
    }
    else
      *errmsg = ubpf_error("internal uBPF error: JIT file open failed: %s\n", strerror(errno));
#endif

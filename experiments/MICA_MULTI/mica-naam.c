...
uint64_t prog(struct xdp_md *ctx)
{
  ...
  if (kv->cmd == REQT_SET)
  {
    ...
    uint64_t key_hash;
    uint64_t kv_nseq = ((req_pkt_t *)pkt)->nseq;
    ...
    DPrintf("SET %lu: BEGIN KEY -> %lu VALUE -> %lu\n", pid, kv->key, kv->val);
    DPrintf("SET %lu: BUCKET -> %u\n", pid, bucket_index);
    ...
    DPrintf("SET %lu: BUCKET -> %u VERSION -> %u\n", pid, bucket_index, current_bucket->version);
    DPrintf("SET %lu: KEY -> %lu\n", pid, kv->key);
    ...
    if (ret == MEHCACHED_ITEMS_PER_BUCKET + 1){
      ...
      *(uint64_t *)((char *)APP_REGION_PTR(pkt) + sizeof(kv_pair)) = item_vec;
      ...
      /* Dedup check: read existing item and compare nseq under bucket lock */
      offset = sizeof(uint64_t) + sizeof(struct mehcached_bucket) * NUM_BUCKETS
               + MEHCACHED_ITEM_OFFSET(item_vec);
      addr_from = DMA_ADDR(1, 1, offset);
      addr_to   = DMA_ADDR(1, 0, APP_REGION_OFFSET(pkt) + sizeof(kv_pair));
      if (DMA_READ(pkt, addr_to, addr_from, sizeof(struct mehcached_item)) != RET_DMA_SUCCESS)
      {
        DPrintf("SET %lu: EXISTING ITEM READ FAILED KEY -> %lu\n", pid, kv->key);
        kv->status = HT_ERR_FATAL;
        goto end;
      }
      struct mehcached_item *existing_item =
          (struct mehcached_item *)((char *)APP_REGION_PTR(pkt) + sizeof(kv_pair));
      if (existing_item->nseq >= kv_nseq)
      {
        DPrintf("SET %lu: DEDUP DROP existing_nseq=%lu >= kv_nseq=%lu\n",
                pid, existing_item->nseq, kv_nseq);
        DPrintf("SET %lu: KEY -> %lu\n", pid, kv->key);
        /* Duplicate: unlock bucket */
        addr_from = DMA_ADDR(1, 1, BUCKET_OFFSET(bucket_index));
        ret = DMA_CAS(pkt, addr_from, cur_version, cur_version + 1);
        /* Duplicate: Drop the packet without sending a reply */
        // return 1;
        /* Set duplicate status if reply is still desired */
        kv->status = HT_ERR_DUPLICATE;
        goto end;
      }
    }

    struct mehcached_item item = {.key_hash = key_hash, .key = kv->key, .val = kv->val, .nseq = kv_nseq};
    ...
    *(struct mehcached_item *)((char *)APP_REGION_PTR(pkt) + sizeof(kv_pair)) = item;
    ...
    if (ret == cur_version)
    {
      DPrintf("SET %lu: VERSION_UPDATE SUCCESSFUL KEY -> %lu\n", pid, kv->key);
      DPrintf("SET %lu: O_VERSION -> %u N_VERSION -> %u\n", pid, cur_version, cur_version + 1);
    } else {
      DPrintf("SET %lu: VERSION_UPDATE FAILED OLD VERSION -> %u RET -> %u\n", pid, cur_version, ret);
      DPrintf("SET %lu: KEY -> %lu\n", pid, kv->key);
  ...
  else // READ
  {
    ...
    for (version_counter = 0; version_counter < 2; version_counter++)
    {
      ...
      DPrintf("GET %lu: BUCKET -> %u VERSION -> %u\n", pid, bucket_index, current_bucket->version);
      DPrintf("GET %lu: KEY -> %lu\n", pid, key);
    ...
    for (item_index = 0; item_index < MEHCACHED_ITEMS_PER_BUCKET; item_index++)
    {
      ...
      {
        DPrintf("GET %lu: HASH FAILED KEY -> %lu ITEM_H -> %lu\n", pid, key, item->key_hash);
        DPrintf("GET %lu: KEY_H -> %lu\n", pid, key_hash);
        continue;
      }

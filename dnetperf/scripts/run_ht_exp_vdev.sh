...
while [ $PPS_START -le $PPS_END ]; do
	...
	for i in $(seq 1 $NUM_ITER); do
		sudo "$CLIENT_BIN" -l $LCORES --no-pci --vdev=net_virtio_user0,path=/tmp/my_vhost.sock,queues=10 -- -s 192.168.1.2 -M "$SERVER_MAC" -p 10002 -t 10 -T ht --workload C --key-dist uniform -b 32 -n 1 -m fixed -r $PPS_START --src-port-start 1234 --src-port-end 1243 &>> "$LOG_FILE"
	done

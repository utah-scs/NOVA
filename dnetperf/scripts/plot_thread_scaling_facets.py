...
def read_peak_dpu_host(path, dpu_path=None):
    ...
    if dpu_path:
        dpu_df = pd.read_csv(os.path.abspath(dpu_path))
        dpu_df["threads"] = 0
    ...
    series_frames = [
        peak.assign(series="NIC", throughput_mpps=peak["dpu_mpps"]),
        peak.assign(series="Host", throughput_mpps=peak["host_mpps"]),
        peak.assign(series="Combined", throughput_mpps=peak["combined_mpps"]),
    ]

    tidy = pd.concat(series_frames, ignore_index=True)
...
def parse_args():
    ...
    parser.add_argument(
        "--dpu",
        default="results/thread_scaling_dpu_only/results.csv",

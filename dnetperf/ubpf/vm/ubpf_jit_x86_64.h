...
/* test rax, imm32 */
static inline void
emit_test(struct jit_state *state, int32_t imm)
{
    emit1(state, 0x48);
    emit1(state, 0xa9);
    emit4(state, imm);
...
static inline void
emit_je(struct jit_state *state, int32_t offset)
{
    emit1(state, 0x0f);
    emit1(state, 0x84);
    emit4(state, offset);
...
static inline void
emit_jl(struct jit_state *state, uint8_t offset)
{
    emit1(state, 0x7c);
    emit1(state, offset);
}
...
static inline void
emit_jg(struct jit_state *state, uint8_t offset)
{
    emit1(state, 0x7f);
    emit1(state, offset);
}
...
/* emit indirect register jump "jmp src" */
static inline void
emit_jmp_reg(struct jit_state *state, int src)
{
   emit_basic_rex(state, 0, src, 0);
   emit1(state, 0xff);
   emit_modrm(state, 0xc0, 4, src);
}

/* emit "lea    dst,[rip+0x0]" */
static inline void
emit_lea_rip(struct jit_state *state, int dst)
{
    emit_rex(state, 1, !!(dst & 8), 0, 0);
    emit1(state, 0x8d);
